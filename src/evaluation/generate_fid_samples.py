...
def load_ddpm(checkpoint_path, device, config_path=None):
    ...
    if cfg is None:
        raise RuntimeError("DDPM config fehlt. Starte das Script mit --ddpm-config.")
...
def load_classifier(checkpoint_path, device, config_path=None):
    ...
    if looks_like_noisy_unet_classifier(state_dict):
        ...
        raise RuntimeError("Classifier config fehlt. Guidance > 0 ist nicht möglich.")
...
def scale_to_name(scale):
    if float(scale).is_integer():
        return f"scale_{int(scale)}"

    return f"scale_{str(scale).replace('.', 'p')}"


def save_individual_images(samples, out_dir, start_index):
    out_dir.mkdir(parents=True, exist_ok=True)

    for i, image in enumerate(samples):
        image_index = start_index + i
        image_path = out_dir / f"sample_{image_index:05d}.png"
        save_image(image, image_path)


def save_preview(samples, out_dir, max_preview_images=16):
    if samples.shape[0] == 0:
        return

    preview = samples[:max_preview_images]
    nrow = int(math.sqrt(preview.shape[0]))

    if nrow * nrow != preview.shape[0]:
        nrow = min(preview.shape[0], 4)

    grid = make_grid(preview, nrow=nrow, padding=2)
    save_image(grid, out_dir / "preview_grid.png")
...
def generate_batch(
    ...
    init_generator = make_generator(device, seed)
    initial_noise = torch.randn(shape, device=device, generator=init_generator)

    step_generator = make_generator(device, seed + 1_000_000)
...
def main():
    ...
    parser.add_argument("--ddpm-config", type=str, required=True)
    ...
    parser.add_argument("--class-labels", type=str, default="0,1,5,8")
    parser.add_argument("--guidance-scales", type=str, default="0,5,10")
    parser.add_argument(
        "--guidance-schedule",
        type=str,
        default="sin",
        choices=["constant", "const", "sin", "sine", "sin2", "sine2", "sin^2", "sine_squared"],
        help="Timestep-dependent guidance schedule passed to DDPMSampler.sample.",
    )
    parser.add_argument("--num-images-per-class", type=int, default=1000)
    parser.add_argument("--batch-size", type=int, default=32)

    parser.add_argument("--seed", type=int, default=999)
    parser.add_argument("--out-dir", type=str, default="outputs/generated_fid")
    ...
    if any(scale > 0 for scale in guidance_scales) and args.classifier_checkpoint is None:
        raise ValueError("--classifier-checkpoint is required for guidance scales > 0.")

    out_root = Path(args.out_dir)
    out_root.mkdir(parents=True, exist_ok=True)

    print("=" * 80)
    print("FID sample generation")
    print("=" * 80)
    print(f"Device:                 {device}")
    print(f"DDPM checkpoint:        {args.ddpm_checkpoint}")
    print(f"DDPM config:            {args.ddpm_config}")
    print(f"Classifier checkpoint:  {args.classifier_checkpoint}")
    print(f"Classifier config:      {args.classifier_config}")
    print(f"Output dir:             {out_root}")
    print(f"Classes:                {class_labels}")
    print(f"Guidance scales:        {guidance_scales}")
    print(f"Guidance schedule:      {args.guidance_schedule}")
    print(f"Images per class/scale: {args.num_images_per_class}")
    ...
    if classifier is not None:
        ...
        print("No classifier loaded. Only scale 0 is possible.")
    ...
    total_jobs = len(class_labels) * len(guidance_scales)
    ...
    for guidance_scale in guidance_scales:
        for class_label in class_labels:
            ...
            class_out_dir = (
                out_root
                / scale_to_name(guidance_scale)
                / f"{class_label}_{class_name}"
            )

            class_out_dir.mkdir(parents=True, exist_ok=True)

            existing = sorted(class_out_dir.glob("sample_*.png"))
            start_index = len(existing)

            if start_index >= args.num_images_per_class:
                print(
                    f"Skipping class {class_label}={class_name}, "
                    f"scale={guidance_scale}: already has {start_index} images."
                )
                continue

            remaining = args.num_images_per_class - start_index
            num_batches = math.ceil(remaining / args.batch_size)

            print("-" * 80)
            print(f"Class:          {class_label} = {class_name}")
            print(f"Guidance scale: {guidance_scale}")
            print(f"Output:         {class_out_dir}")
            print(f"Existing:       {start_index}")
            print(f"Generating:     {remaining}")
            print("-" * 80)
            ...
            for batch_idx in tqdm(
                range(num_batches),
                desc=f"class={class_label}, scale={guidance_scale}",
            ):
                current_index = start_index + batch_idx * args.batch_size
                current_batch_size = min(
                    args.batch_size,
                    args.num_images_per_class - current_index,
                )

                # Important:
                # Seed does NOT include guidance_scale.
                # Therefore sample_00000 for scale 0, 5, and 10 starts from
                # the same initial noise and uses the same reverse-step noise.
                sample_seed = (
                    args.seed
                    + class_label * 100_000
                    + current_index
                ...
                if len(preview_images) < 16:
                    needed = 16 - len(preview_images)
                    preview_images.extend([img for img in samples[:needed]])

            if len(preview_images) > 0:
                preview_tensor = torch.stack(preview_images, dim=0)
                save_preview(preview_tensor, class_out_dir, max_preview_images=16)

            print(f"Finished class {class_label}={class_name}, scale={guidance_scale}")

    print("=" * 80)
    print("Sampling complete.")
    print(f"Samples saved to: {out_root}")
    print("=" * 80)
