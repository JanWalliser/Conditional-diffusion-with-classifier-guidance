...
def valid_group_count(channels: int, max_groups: int = 32) -> int:
    """
    Returns the largest valid GroupNorm group count <= max_groups.
    """
...
def make_num_heads(channels: int, channels_per_head: int = 64) -> int:
    """
    Chooses a valid number of attention heads.
    Tries to use approximately channels_per_head channels per head.
    """
...
class ResBlock(nn.Module):
    """
    UNet-style residual block with timestep conditioning.

    Uses scale-shift conditioning:
        h = norm(h) * (1 + scale(t)) + shift(t)

    This is closer to the ADM/UNet style than your previous plain ResNet classifier.
    """
...
class AttentionBlock(nn.Module):
    """
    Spatial self-attention block for low-resolution feature maps.
    Intended mainly for 8x8 and optionally 16x16.
    """
...
class AttentionPool2d(nn.Module):
    ...
    This replaces simple global average pooling and is closer to the paper idea.
    """
    ...
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(
            torch.randn(spatial_dim * spatial_dim + 1, embed_dim) / math.sqrt(embed_dim)
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if h != self.spatial_dim or w != self.spatial_dim:
            raise ValueError(
                f"AttentionPool2d expected spatial size "
                f"{self.spatial_dim}x{self.spatial_dim}, got {h}x{w}."
            )
...
class NoisyUNetClassifier(nn.Module):
    """
    UNet-downsampling classifier for classifier guidance.
    ...
    This is intended for CIFAR-10 classifier guidance:
        p_phi(y | x_t, t)
    """
    ...
    def _init_weights(self) -> None:
        ...
        # Small output head init is usually more stable.
