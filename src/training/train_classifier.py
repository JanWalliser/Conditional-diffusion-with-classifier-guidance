...
def build_timestep_sampling_probs(
    ...
    sine: probability mass is concentrated in the medium-noise region and
    reduced at low/high noise:
        w(t) = floor + sin(pi * (t + 0.5) / T) ** power

    linear_decay: per-timestep weight decreases from low-noise to high-noise.
    Note that printed bins have unequal widths, so a wider middle bin may still
    have more total mass although its per-timestep density is lower.
    """
    strategy = str(
    ...
    elif strategy in {"linear_decay", "linear-decay", "linear"}:
        # More low-noise classifier updates, fewer high-noise updates.
        # min_weight controls the high-noise end of the distribution.
        min_weight = float(sampling_cfg.get("min_weight", 0.1))
        if min_weight < 0:
            raise ValueError(
                f"timestep_sampling.min_weight must be >= 0, got {min_weight}"
            )
        weights = torch.linspace(1.0, min_weight, num_timesteps, device=device)

    else:
        raise ValueError(
            f"Unknown timestep sampling strategy: {strategy}. "
            "Supported strategies: uniform, sine, linear_decay."
        )
...
def print_timestep_sampling_distribution(
    ...
    for lo, hi in bins:
        ...
        width = hi_clamped - lo_clamped
        density = mass / max(width, 1)
        print(
            f"  t[{lo_clamped},{hi_clamped}) : mass={mass:.4f}, "
            f"per-timestep={density:.7f}"
        )
...
def main() -> None:
    ...
    fixed_timesteps = validation_cfg.get(
        "fixed_timesteps",
        train_cfg.get("fixed_timesteps", None),
    )
    if fixed_timesteps is not None:
        fixed_timesteps = [int(t) for t in fixed_timesteps]
